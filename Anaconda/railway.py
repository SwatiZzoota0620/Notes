...
def create():
    ...
    c=con.cursor()
    tb_create='''SELECT table railway(from text,to text, preferred_airline text, prefered_seating text, deaparture_date number, departure_time number, age(12+ Yrs) number, childrean(2-11 yrs) number, infant number, select_your_face text, return_date number, return_time number, full_name text, phone_number number, email text)'''
    tb_exists="SELECT name from sqlite_master where type='table' and name='railway'"
    if not c.execute(tb_create):
        c.execute(tb_create)
        con.commit()
    con.commit()
    con.close()
...
v= StringVar()
entry3 = ttk.Combobox(screen2, width = 27, textvariable = v, values=(' IndiGo',  ' SpiceJet', ' Air India',  ' AirGo')).place(x=170,y=120)
